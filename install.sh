...
install_dir="${GMIND_BIN_DIR:-${HOME}/.local/bin}"
codex_skill_dir="${GMIND_CODEX_SKILL_DIR:-${HOME}/.agents/skills/gmind}"
claude_skill_dir="${GMIND_CLAUDE_SKILL_DIR:-${HOME}/.claude/skills/gmind}"

info "檢查 gmind 安裝環境..."

[ -f "${repo_dir}/pyproject.toml" ] || fail "請在解壓縮後的 gmind 專案資料夾裡執行 install.sh。"

if ! have uv; then
  if [ "${GMIND_AUTO_INSTALL_UV:-1}" = "0" ]; then
    fail "找不到 uv。請先安裝 uv，或移除 GMIND_AUTO_INSTALL_UV=0 後讓本腳本自動安裝。"
  fi
...
if [ "$(uname -s)" != "Darwin" ]; then
  ...
  info "其他系統仍可使用手動 token 設定：gmind auth set-token"
fi
...
#!/usr/bin/env bash
cd "$repo_dir"
exec "$uv_bin" run gmind "\$@"
EOF

chmod +x "${install_dir}/gmind"
info "已安裝 gmind 到 ${install_dir}/gmind"

if [ -f "${repo_dir}/SKILL.md" ]; then
  if [ "${GMIND_INSTALL_CODEX_SKILL:-1}" != "0" ]; then
    mkdir -p "$codex_skill_dir"
    cp "${repo_dir}/SKILL.md" "${codex_skill_dir}/SKILL.md"
    info "已安裝 Codex skill 到 ${codex_skill_dir}/SKILL.md"
  fi

  if [ "${GMIND_INSTALL_CLAUDE_SKILL:-1}" != "0" ]; then
...
"${install_dir}/gmind" --help >/dev/null
info "smoke test 通過：gmind --help"
